...
class _PickupScreenNotificationState extends State<PickupScreenNotification> {
  ...
  void initState() {
    super.initState();
    // _getCurrentLocation() को पूरी तरह हटा दिया गया
    socket = widget.socket ??
        IO.io('https://backend.weloads.live', <String, dynamic>{
          'transports': ['websocket'],
          'autoConnect': false,
  ...
  void _emitDriverArrivedAtPickup() async {
    ...
    if (socket.connected) {
      ...
      socket.on("delivery:status_update", (data) {
        ...
        if (status == "arrived") {
          final waitingTime = data["waitingTime"];
          final arrivedAt = data["arrivedAt"];
          int freeMinutes = 5;
          if (waitingTime != null) {
            freeMinutes = int.tryParse(waitingTime.toString()) ?? 5;
  ...
  Future<void> _openCustomerLiveTracking() async {
    if (deliveryData?.data?.pickup == null || deliveryData?.data?.dropoff?.isEmpty == true) {
      Fluttertoast.showToast(msg: "Route not available");
      return;
    }

    final pickup = deliveryData!.data!.pickup!;
    final lastDrop = deliveryData!.data!.dropoff!.last;

    String waypoints = deliveryData!.data!.dropoff!
        .take(deliveryData!.data!.dropoff!.length - 1)
        .map((d) => '${d.lat},${d.long}')
        .join('|');
    ...
    final url = Uri.parse(
      'https://www.google.com/maps/dir/?api=1'
          ...
          '&destination=${lastDrop.lat},${lastDrop.long}'
  ...
  Widget build(BuildContext context) {
    ...
    // Pickup location के लिए fallback (map center के लिए)
    final LatLng initialTarget = pickup?.lat != null && pickup?.long != null
        ? LatLng(pickup!.lat!, pickup.long!)
        ...
        body: Stack(
          children: [
            GoogleMap(
              ...
              markers: _markers,
              polylines: _polylines,
              myLocationEnabled: false,        // Customer location disabled
              myLocationButtonEnabled: false,  // Blue dot button hidden
            ),
              ...
              builder: (context, scrollController) {
                return Container(
                  ...
                  child: ListView(
                    ...
                    padding: EdgeInsets.symmetric(horizontal: 20.w, vertical: 12.h),
                    children: [
                      // ... बाकी सारा UI पहले जैसा ही है (कोई बदलाव नहीं)
                      Center(
                        child: Container(
                          width: 40.w,
                          height: 5.h,
                      ...
                      if (!_isArrived)
                        ...
                        Container(
                          ...
                          width: double.infinity,
                          decoration: BoxDecoration(
                            color: _waitingSeconds >= _maxFreeWaitingSeconds ? Colors.red.shade600 : Colors.blue.shade600,
                            borderRadius: BorderRadius.circular(12.r),
                            boxShadow: [
                              BoxShadow(
                                color: (_waitingSeconds >= _maxFreeWaitingSeconds ? Colors.red : Colors.blue).withOpacity(0.4),
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Column(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  Text("Waiting Time", style: TextStyle(color: Colors.white70, fontSize: 13.sp)),
                                  Text(_waitingTimeText, style: TextStyle(color: Colors.white, fontSize: 24.sp, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                                ],
                              ),
                            ],
                          ),
...
// CancelBottomSheetContent class unchanged (आपके पास पहले से है)
class CancelBottomSheetContent extends StatefulWidget {
  final String txId;
  final VoidCallback onCancel;
  const CancelBottomSheetContent({super.key, required this.txId, required this.onCancel});
