...
class _SelectTripScreenState extends ConsumerState<SelectTripScreen> {
  ...
  void _addNearbyDriverMarkers() {
    if (nearbyDrivers.data == null || nearbyDrivers.data!.isEmpty || !_iconsLoaded) return;
  ...
  Future<void> _fetchMultiStopRoute() async {
    ...
    for (int i = 0; i < dropLats.length; i++) {
      ...
      } catch (e) { log("Drop route $i error: $e"); }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      //     final dropAddress = dropNames.join(" → ");
      //
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        children: [
                          ...
                          // SizedBox(height: 200.h, child: _buildVehicleList(snp)),

                          SizedBox(height: 200.h, child: _buildVehicleList(response)),
                          // _buildPaymentRow(),
  ...
  Widget _buildVehicleList(dynamic snp) {
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: isSelected ? Colors.black : Colors.grey.shade400, width: isSelected ? 1.5 : 1),
              boxShadow: [BoxShadow(color: isSelected ? Colors.black26 : Colors.black12, blurRadius: isSelected ? 8 : 4, offset: const Offset(0, 4))],
            ),
            child: Column(
              children: [
                ...
                const SizedBox(height: 10),
                AnimatedScale(scale: isSelected ? 1.15 : 1.0, duration: const Duration(milliseconds: 250), child: Image.network(snp.data[index].image, width: 116.w, height: 70.h, fit: BoxFit.contain)),
                Padding(padding: const EdgeInsets.only(left: 10, top: 10), child: Text(snp.data[index].name, style: GoogleFonts.inter(fontSize: isSelected ? 17.sp : 15.sp, fontWeight: FontWeight.w600))),
                Padding(padding: const EdgeInsets.only(left: 10), child: Text("₹${snp.data[index].price}", style: GoogleFonts.inter(fontSize: isSelected ? 16.sp : 14.sp))),
              ],
            ),
          ),
        );
      ...
      onPressed: isBooking ? null : () async {
        ...
        try {
          final body = BookInstantDeliveryBodyModel(
            ...
            coinAmount: 0,
            copanId: null,
            dropoff: selectedVehicle.dropoff?.map((d) => BookDropoff(
              name: d.name?.toString() ?? "Drop ${selectedVehicle.dropoff!.indexOf(d) + 1}",
              lat: d.lat ?? 0.0,
              long: d.long ?? 0.0,
            )).toList() ??
                [],
            ...
            userPayAmount: double.parse(selectedVehicle.price ?? "0").toInt(),
            taxAmount: 18.0, // या API से ले सकते हैं
            mobNo: phon,
            name: "nmbkj"
          );
      ...
      onPressed: isBooking ? null : () async {
        ...
        try {
          final body = BookInstantDeliveryBodyModel(
            ...
            copanId: null,
            dropoff: selectedVehicle.dropoff?.map((d) => BookDropoff(
              name: d.nameString, // ← यहाँ nameString
              lat: d.lat ?? 0.0,
              long: d.long ?? 0.0,
            )).toList() ??
                [],
          ...
          //         dropLons: dropLons,
          //         dropNames: dropNames,
          //         txId: response.data!.txId, // ← सिर्फ txId
          //         // body: body,
          //         // socket: socket!,
          //         // pickupLat: pickupLat,
          //         // pickupLon: pickupLon,
      ...
      onPressed: isBooking ? null : () async {
        ...
        try {
          ...
          if (response.code == 0 ) {
            ...
            final errorMsg = response.message ?? "Invalid response from server";
            log("Booking failed: $errorMsg | data type: ${response.data.runtimeType}");
...
class _WaitingForDriverScreenState extends State<WaitingForDriverScreen>
  ...
  void _retrySearch() async {
    ...
    try {
      final body = BookInstantDeliveryBodyModel(
        vehicleTypeId: "68ce853be9401176157710f7", // Ya current selected vehicle se
    ...
    return Scaffold(
      ...
      body: _routeFetched && _iconsLoaded
        ...
        children: [
          ...
          // Route Info
          Positioned(
            bottom: 170.h, left: 16.w, right: 16.w,
            child: Container(
              padding: EdgeInsets.all(12.w),
              decoration: BoxDecoration(
                color: Colors.white,
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    _isSearching ? "Searching for nearby drivers$dots" : "No drivers found nearby",
                    style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.black87),
                    textAlign: TextAlign.center,
                  ),
                  const SizedBox(height: 12),
                  _isSearching
                      ? Text("Please wait... (${_formatTime(_remainingSeconds)})", style: const TextStyle(fontSize: 15, color: Colors.grey), textAlign: TextAlign.center)
                      : const Text("Try again or cancel request.", style: TextStyle(fontSize: 15, color: Colors.grey), textAlign: TextAlign.center),
